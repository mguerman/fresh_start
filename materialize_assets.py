...
def materialize_batch(batch_num, total_batches, assets_batch, resources):
    ...
    unresolved_job = define_asset_job(name=job_name, selection=assets_batch)

    # Create Definitions and register the unresolved job
    defs = Definitions(
        assets=assets_batch,
        resources=resources,
        jobs=[unresolved_job]
    ...
    try:
        # Resolve the job before execution
